...
export default function SslBatchPage() {
  const t = useTranslations("SslBatch");
  const common = useTranslations("Common");

  const FORM_KEY = buildFormCacheKey("ssl.batch");
  ...
  const [alwaysUseHttps, setAlwaysUseHttps] = useState<string>("");
  const [minTlsVersion, setMinTlsVersion] = useState<string>("");
  const [tls13, setTls13] = useState<string>("");
  const [automaticHttpsRewrites, setAutomaticHttpsRewrites] = useState<string>("");
  const [opportunisticEncryption, setOpportunisticEncryption] = useState<string>("");
  const [universalSsl, setUniversalSsl] = useState<string>("");
  ...
  const tlsVersionOptions = useMemo(() => ["", "1.0", "1.1", "1.2", "1.3"], []);
  ...
  async function onStart() {
    ...
    const settings = {
      ...
      always_use_https: triToBool(alwaysUseHttps),
      min_tls_version: minTlsVersion || null,
      tls_1_3: triToBool(tls13),
      automatic_https_rewrites: triToBool(automaticHttpsRewrites),
      opportunistic_encryption: triToBool(opportunisticEncryption),
      universal_ssl: triToBool(universalSsl),
    };

    const hasAny =
      settings.ssl_mode ||
      settings.always_use_https !== null ||
      settings.min_tls_version ||
      settings.tls_1_3 !== null ||
      settings.automatic_https_rewrites !== null ||
      settings.opportunistic_encryption !== null ||
      settings.universal_ssl !== null;
    if (!hasAny) {
      setError(t("parseError"));
      return;
    }
    ...
    setRunning(true);
    try {
      const data = await apiRequest<TaskCreateResponse>("/api/v1/ssl/batch", {
        method: "POST",
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      setSslMode(cached.sslMode || "");
      setAlwaysUseHttps(cached.alwaysUseHttps || "");
      setMinTlsVersion(cached.minTlsVersion || "");
      setTls13(cached.tls13 || "");
      setAutomaticHttpsRewrites(cached.automaticHttpsRewrites || "");
      setOpportunisticEncryption(cached.opportunisticEncryption || "");
      setUniversalSsl(cached.universalSsl || "");
    }
      ...
      <div className="rounded-[28px] zen-surface p-6">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="space-y-2">
            <Label htmlFor="sslMode" className="text-sm tracking-wide">
              {t("sslMode")}
            </Label>
            <select
              id="sslMode"
          ...
          <div className="space-y-2">
            <Label htmlFor="minTls" className="text-sm tracking-wide">
              {t("minTlsVersion")}
            </Label>
            <select
              id="minTls"
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="space-y-2">
            <Label htmlFor="alwaysHttps" className="text-sm tracking-wide">
              {t("alwaysUseHttps")}
            </Label>
            <select
              id="alwaysHttps"
              value={alwaysUseHttps}
              onChange={(e) => setAlwaysUseHttps(e.target.value)}
              className="zen-surface-subtle h-14 w-full rounded-xl px-4 text-lg text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring/25 focus-visible:ring-offset-2"
              disabled={running}
            >
              {triOptions.map((x) => (
          ...
          <div className="space-y-2">
            <Label htmlFor="tls13" className="text-sm tracking-wide">
              {t("tls13")}
            </Label>
            <select
              id="tls13"
              value={tls13}
              onChange={(e) => setTls13(e.target.value)}
              className="zen-surface-subtle h-14 w-full rounded-xl px-4 text-lg text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring/25 focus-visible:ring-offset-2"
              disabled={running}
            >
              {triOptions.map((x) => (
          ...
          <div className="space-y-2">
            <Label htmlFor="autoRewrite" className="text-sm tracking-wide">
              {t("automaticHttpsRewrites")}
            </Label>
            <select
              id="autoRewrite"
              value={automaticHttpsRewrites}
              onChange={(e) => setAutomaticHttpsRewrites(e.target.value)}
              className="zen-surface-subtle h-14 w-full rounded-xl px-4 text-lg text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring/25 focus-visible:ring-offset-2"
              disabled={running}
            >
              {triOptions.map((x) => (
          ...
          <div className="space-y-2">
            <Label htmlFor="opEn" className="text-sm tracking-wide">
              {t("opportunisticEncryption")}
            </Label>
            <select
              id="opEn"
              value={opportunisticEncryption}
              onChange={(e) => setOpportunisticEncryption(e.target.value)}
              className="zen-surface-subtle h-14 w-full rounded-xl px-4 text-lg text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring/25 focus-visible:ring-offset-2"
              disabled={running}
            >
              {triOptions.map((x) => (
          ...
          <div className="space-y-2">
            <Label htmlFor="universalSsl" className="text-sm tracking-wide">
              {t("universalSsl")}
            </Label>
            <select
              id="universalSsl"
              value={universalSsl}
              onChange={(e) => setUniversalSsl(e.target.value)}
              className="zen-surface-subtle h-14 w-full rounded-xl px-4 text-lg text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring/25 focus-visible:ring-offset-2"
              disabled={running}
            >
              {triOptions.map((x) => (
