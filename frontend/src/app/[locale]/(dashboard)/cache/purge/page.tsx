...
export default function CachePurgePage() {
  const t = useTranslations("CachePurge");
  const common = useTranslations("Common");

  const FORM_KEY = buildFormCacheKey("cache.purge");
  ...
  async function onStart() {
    ...
    if (!domains.length || !confirm) {
      setError(t("parseError"));
      return;
    }

    const files = parseLines(filesText);
    setRunning(true);
    try {
      const data = await apiRequest<TaskCreateResponse>("/api/v1/cache/purge", {
        method: "POST",
        body: JSON.stringify({ account_id: accountId, confirm: true, domains, files: files.length ? files : null }),
      });
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      setFilesText(cached.filesText || "");
      setConfirm(Boolean(cached.confirm));
    }
  ...
  useEffect(() => {
    saveFormCache(FORM_KEY, { v: 1, accountId, domainsText, filesText, confirm, taskId: taskId || undefined });
  }, [FORM_KEY, accountId, confirm, domainsText, filesText, taskId]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-[28px] zen-surface p-6">
        ...
        <div className="mt-6 space-y-2">
          <Label htmlFor="files" className="text-sm tracking-wide">
            {t("files")}
          </Label>
          <textarea
            id="files"
            ...
            className="zen-surface-subtle min-h-24 w-full rounded-xl p-4 text-lg text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring/25 focus-visible:ring-offset-2"
            disabled={running}
          />
          <div className="text-xs text-muted-foreground">{t("filesHint")}</div>
        </div>
