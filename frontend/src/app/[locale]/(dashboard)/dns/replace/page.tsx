...
export default function DnsReplacePage() {
  const t = useTranslations("DnsReplace");
  const common = useTranslations("Common");

  const FORM_KEY = buildFormCacheKey("dns.replace");
  ...
  async function onStart() {
    ...
    if (!domains.length || !recordType || !recordName || !oldValue.trim() || !newValue.trim()) {
    ...
    setRunning(true);
    try {
      const data = await apiRequest<TaskCreateResponse>("/api/v1/dns/replace", {
        method: "POST",
        body: JSON.stringify({
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      setOldValue(cached.oldValue || "");
      setNewValue(cached.newValue || "");
  ...
  useEffect(() => {
    saveFormCache(FORM_KEY, { v: 1, accountId, recordType, recordName, oldValue, newValue, domainsText, taskId: taskId || undefined });
  }, [FORM_KEY, accountId, domainsText, newValue, oldValue, recordName, recordType, taskId]);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[28px] zen-surface p-6">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <div className="space-y-2">
            <Label htmlFor="oldValue" className="text-sm tracking-wide">
              {t("oldValue")}
            </Label>
            <Input
              id="oldValue"
          ...
          <div className="space-y-2 md:col-span-2">
            <Label htmlFor="newValue" className="text-sm tracking-wide">
              {t("newValue")}
            </Label>
            <Input
              id="newValue"
