...
export default function DomainsAddPage() {
  const t = useTranslations("DomainAdd");
  const common = useTranslations("Common");

  const FORM_KEY = buildFormCacheKey("domains.add");
    ...
    eventSourceRef.current = openTaskEventSource<ProgressEventData, CompleteEventData>(
      ...
      createBatchTaskSseHandlers<ProgressEventData, CompleteEventData, TaskLogRow>({
        ...
        buildLogRow: (data) => {
          ...
          if (data.zone_status) details.push(String(data.zone_status));
          if (data.name_servers?.length) details.push(data.name_servers.join(", "));
          return {
            domain: data.domain!,
            status: data.status!,
  ...
  async function onStart() {
    ...
    setRunning(true);
    try {
      const data = await apiRequest<TaskCreateResponse>("/api/v1/domains/add", {
        method: "POST",
